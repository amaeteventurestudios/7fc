...
import { Reveal, SectionTitle, CountUp, PlaceholderImg } from "./ui";
...
export function Journey() {
  return (
    <section id="journey" className="section-glow py-16 md:py-24 px-4">
      <div className="mx-auto max-w-6xl">
        <SectionTitle>The Journey</SectionTitle>
        <div className="relative flex flex-col md:flex-row md:justify-between gap-8 md:gap-2">
          <div
            className="absolute md:top-10 md:left-0 md:right-0 md:h-px md:w-full top-0 bottom-0 left-1/2 w-px bg-gradient-to-b md:bg-gradient-to-r from-transparent via-gold/50 to-transparent"
            aria-hidden
          />
          {JOURNEY.map((j, i) => (
            <Reveal key={j.name} delay={i * 100} className="relative md:flex-1">
              <div className="flex flex-col items-center text-center px-2">
                <PlaceholderImg
                  ...
                  className="w-14 h-14 rounded-full border border-gold/30 object-cover"
                  label={j.name}
                />
                <span className="pulse-node mt-3 block w-2.5 h-2.5 rounded-full bg-gold" aria-hidden />
                <h3 className="mt-3 font-display text-xs font-bold tracking-[0.15em] uppercase text-gold-2">
                  {j.name}
                </h3>
                <p className="text-[11px] text-electric mt-1">{j.years}</p>
                <p className="text-[11px] text-gray-400 mt-1 leading-snug">{j.copy}</p>
              </div>
            </Reveal>
          ))}
        </div>
              ...
              <Reveal key={c.era} delay={i * 60}>
                <button
                  ...
                  className={`glass-card w-full overflow-hidden text-center transition-all ${
                    selected
                      ? "border-gold shadow-[0_0_24px_rgba(212,175,94,0.45)]"
                ...
                >
                  ...
                  />
                  <div className="p-3">
                    <h3 className="font-display text-[11px] font-bold tracking-widest uppercase text-gold-2">
                      {c.era}
                    </h3>
                    <p className="text-[10px] text-gray-400 mt-1">{c.tagline}</p>
                  </div>
                </button>
              </Reveal>
            );
...
export function Moments() {
  return (
    <section id="moments" className="section-glow py-16 md:py-24 px-4">
      <div className="mx-auto max-w-6xl">
        <SectionTitle>7FC Moments</SectionTitle>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {MOMENTS.map((m, i) => (
            <Reveal key={m.title} delay={i * 80}>
              <div className="glass-card h-full overflow-hidden text-center hover:border-gold/50 transition-colors">
                <PlaceholderImg
                  src={m.img}
                ...
                />
                <div className="p-4">
                  <h3 className="font-display text-xs font-bold tracking-[0.15em] uppercase text-gold-2">
                    {m.title}
                  </h3>
                  <p className="text-[11px] text-gray-400 mt-2">{m.copy}</p>
                </div>
              </div>
            </Reveal>
          ))}
...
export function RecordWall() {
  return (
    <section id="records" className="py-16 md:py-24 px-4">
      <div className="mx-auto max-w-6xl">
          ...
          {RECORDS.map((r, i) => (
            <Reveal key={r.top} delay={i * 70}>
              <div className="glass-card h-full p-5 text-center">
                <p className="text-[10px] tracking-[0.2em] uppercase text-gray-400">{r.top}</p>
                <p className="font-display text-2xl md:text-3xl font-bold gold-text mt-2">
                  <CountUp target={r.value} suffix={r.suffix} />
                </p>
                <p className="text-[10px] tracking-[0.2em] uppercase text-electric mt-1">
                  {r.label}
                </p>
              </div>
            </Reveal>
  ...
  return (
    <section className="relative overflow-hidden border-y border-gold/20">
      ...
      <div className="relative mx-auto max-w-4xl px-4 py-16 md:py-24 text-center">
        <span className="font-display text-5xl gold-text leading-none" aria-hidden>
          &ldquo;
        </span>
        <p className="font-display text-xl md:text-3xl text-white font-bold tracking-wide leading-snug">
          Talent opens the door.
          <br />
          Discipline owns the room.
        </p>
        <p className="mt-4 text-crimson text-xs md:text-sm tracking-[0.35em] uppercase font-semibold">
